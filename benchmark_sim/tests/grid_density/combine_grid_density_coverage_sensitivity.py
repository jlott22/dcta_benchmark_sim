#!/usr/bin/env python3
"""Combine COVERAGE grid-density sensitivity raw outputs into one combined directory."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--run-root",
        default=str(REPO_ROOT / "results" / "sensitivity_coverage_grid_density_50"),
    )
                ...
                for col in BACKFILL_COLS:
                    if not row.get(col, ""):
                        if col == "algorithm_name":
                            ...
                            row[col] = cond.get("trial_mode", "coverage")
                        else:
