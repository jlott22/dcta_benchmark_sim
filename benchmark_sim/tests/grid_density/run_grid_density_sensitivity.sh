...
if ! [[ "$WORKERS" =~ ^[0-9]+$ ]] || [[ "$WORKERS" -lt 1 ]]; then
  echo "Usage: bash benchmark_sim/tests/grid_density/run_grid_density_sensitivity.sh [num_workers]" >&2
  exit 2
fi

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/../../.." && pwd)"
RUN_ROOT="$REPO_ROOT/results/sensitivity_clue_search_grid_density_50"
...
echo "  bash benchmark_sim/tests/grid_density/combine_grid_density_sensitivity.sh"
echo "============================================================"
